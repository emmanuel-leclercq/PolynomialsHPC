...
std::vector<T2> find_roots(const Polynomial<T1> &P)
{
    if (P.degree() == 1)
    {
        return std::vector<T2>({-P[1] / P[0]});
    } // P[0] cannot be zero with our construction
    ...
    if (P.degree() == 2)
    {
        ...
        if (delta >= 0)
        {
            return std::vector<T2>({(-P[1] - sqrt(delta) / (2 * P[2])), (-P[1] + sqrt(delta)) / (2 * P[2])});
        }
        else
        {
            ...
            {
                return std::vector<T2>(); // no real roots
            }
        }
    }
...
std::vector<T> Polynomial<T>::RawMultipointEval(const std::vector<T> &points) const
{
    ...
    int m = points.size();
    std::vector<T> ans(m);
    std::ranges::transform(points, ans.begin(), [this](const T &x)
...
Polynomial<decltype(T1() * T2())> operator/(const Polynomial<T1> &p, const T2 q)
{
    ...
    std::ranges::transform(p.coefficients, ans.begin(), [q](const T1 x)
                           { return x / q; });
