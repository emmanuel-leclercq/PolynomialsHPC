...
template <typename T> inline Dense<T> derivative(Dense<T> P, int k = 1);
...
public:
  ...
  template <typename U> std::vector<T> operator()(const std::vector<U> &) const;
...
template <typename T> bool Dense<T>::is_sparse() const {
  int zeros = 0;
...
template <typename T> void Dense<T>::adjust() {
  /*
...
template <typename T> Dense<T> Dense<T>::extend(int m) const {
  if (m > n) {
...
template <typename T> Dense<T> derivative(Dense<T> P, int k) {
  P.derivative(k);
  return P;
}

template <typename T> void Dense<T>::derivative(int k) {
  if (k == 0) [[unlikely]] {
    return;
  }
  if (n < k) {
